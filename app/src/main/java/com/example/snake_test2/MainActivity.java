package com.example.snake_test2;
...
public class MainActivity extends AppCompatActivity implements SurfaceHolder.Callback {
    ...
    private boolean checkGameOver(int head_PositionX, int head_PositionY){
        ...
        //check phần đầu chạm rìa
        if(lstSnakeSegments.get(0).getPositionX() < 0
                || lstSnakeSegments.get(0).getPositionY() < 0
                || lstSnakeSegments.get(0).getPositionX() >= surfaceView.getWidth()
                || lstSnakeSegments.get(0).getPositionY() >= surfaceView.getHeight()) {
            gameOver = true;
        }
        //check tự cắn
        else{
